...
void simulation_cap (int n )
{
	...
	voilier2.setTexture(img_voilier);
	...
	voilier2.scale(0.1, 0.1);
	voilier2.setOrigin(voilier2.getLocalBounds().width /2, voilier2.getLocalBounds().width/2);
    ...
    maj_voilier (etat2, w);
    commande (etat2, w, cap2);
	x = floorf(etat2[0].x);
	y = floorf(etat2[0].y);
	if (etat2[0].x > tx - 5) {etat2[0].x -= tx-5; grille.loadFromFile("grille.png");}
	if (etat2[0].y > ty - 5) {etat2[0].y -= ty-5; grille.loadFromFile("grille.png");}
	if (etat2[0].x <  5) {etat2[0].x += tx-5; grille.loadFromFile("grille.png");}
	if (etat2[0].y <  5) {etat2[0].y += ty-5; grille.loadFromFile("grille.png");}
		
	
		//affichage
		if ( x>5 && y > 5 && x <tx-5 && y< ty-5)
		{
			grille.setPixel(x, y, sf::Color::Red);
			grille.setPixel(x+1, y, sf::Color::Red);
			grille.setPixel(x-1, y, sf::Color::Red);
			grille.setPixel(x, y+1, sf::Color::Red);
			grille.setPixel(x, y-1, sf::Color::Red);
			voilier2.setPosition(x,y);
			voilier2.setRotation(etat2[3].x*180/pi + 90);

			rendu.update(grille);
			affichage.setTexture(rendu);
			window.clear();
			window.draw(affichage);
			window.draw(voilier2);
			window.draw(voilier);
			window.draw(fleche);
			window.display();
		}
